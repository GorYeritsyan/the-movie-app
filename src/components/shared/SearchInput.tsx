...
const SearchInput = () => {
    ...
    // Debounce the search requests prevent making too many API requests
    // by waiting for the use to stop typing for 500ms
    useDebounce(() => setDebouncedInputValue(inputValue), 500, [inputValue])

  const { data: searchedMovies, isFetching } = useSearchMoviesQuery(debouncedInputValue);
